...
Matrix4f Matrix4f::Rotation(const Vec3f &rotation)
{
		...
		float x = rotation.getX() * PI / 180.0;
		float y = rotation.getY() * PI / 180.0;
		float z = rotation.getZ() * PI / 180.0;
...
{
	float tanFOV = tan((fovy/2) * PI / 180.0);
...
Quaternion Matrix4f::mul(const Quaternion &v)
	{
		Quaternion res = Quaternion(0,0,0,0);
